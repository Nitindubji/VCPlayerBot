...
HOME_TEXT = "<b>Hey  [{}](tg://user?id={}) 🙋‍♂️\n\nThis ia a bot made by charts and discussion.\nI Can Stream </b>"
...
async def start(client, message):
    if len(message.command) > 1:
        if message.command[1] == 'help':
            ...
            if message.from_user.id not in Config.ADMINS:
                return await msg.edit("GHANU OP")
    ...
    buttons = [
        [
            InlineKeyboardButton('⚙️ Update Channel', url='https://t.me/'),
...
async def repo_(client, message):
    buttons = [
        [
            InlineKeyboardButton('🧩 Repository', url='https://github.colayerBot'),
            InlineKeyboardButton('⚙️ Update Channel', url='https://tbin_works'),     
        ],
        [
            InlineKeyboardButton("🎞 How to Deploy", url='https://nWgZMrNe_0'),
            InlineKeyboardButton('🗑 Close', callback_data='cose'),
        ]
    ]
    await message.reply("<b>The source code of this bot is public and can be found at <acom/subinps/VCPlayerBot>VCPlayerBot.</a>\nYou can deploy your own bot and use in your group.\n\nFeel free to star☀️ the repo if you liked it 🙃.</b>", reply_markup=InlineKeyboardMarkup(buttons), disable_web_page_preview=True)
    await delete_messages([message])
